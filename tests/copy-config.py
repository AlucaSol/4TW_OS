#!/usr/bin/python3
"""Copy one data partition by GPT coordinates, never a physical disk path."""
from pathlib import Path
import struct
import sys

if len(sys.argv) != 4:
    raise SystemExit(2)
source, destination = map(Path, sys.argv[1:3])
partition = int(sys.argv[3])
expected = {3: "config-write-test.fat", 4: "writing-write-test.fat"}
assert source.is_file() and source.suffix == ".img"
assert partition in expected and destination.name == expected[partition] and destination.parent.name == ".work"
with source.open("rb") as handle:
    handle.seek(512)
    header = handle.read(512)
    assert header[:8] == b"EFI PART"
    table_lba = struct.unpack_from("<Q", header, 72)[0]
    entry_size = struct.unpack_from("<I", header, 84)[0]
    handle.seek(table_lba * 512 + entry_size * (partition - 1))
    entry = handle.read(entry_size)
    first, last = struct.unpack_from("<QQ", entry, 32)
    assert 0 < first < last < source.stat().st_size // 512
    handle.seek(first * 512)
    total = remaining = (last - first + 1) * 512
    with destination.open("wb") as output:
        while remaining:
            data = handle.read(min(1024 * 1024, remaining))
            if not data:
                raise RuntimeError("Truncated image")
            if data.strip(b"\0"):
                output.write(data)
            else:
                output.seek(len(data), 1)
            remaining -= len(data)
        output.truncate(total)
