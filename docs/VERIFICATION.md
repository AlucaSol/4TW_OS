...
The complete `configure` stage assembled the runtime using the existing package
cache, after which `build-img.sh` created one new raw image and
`verify-img.sh` mounted and inspected that exact image. No build step executed a
hardware-clock operation on WSL or Windows.
...
## Final IMG inspection

The current image was attached as a **read-only loop device**, not flashed to a
drive. `artifacts/verify-img.log` records these checks:
...
| Final IMG RTC policy | `/etc/adjtime` is absent; Chrony is denied the RTC device and known hardware-clock save units are masked |
| Local timezone fallback | `/etc/localtime` links to `/usr/share/zoneinfo/Etc/UTC`; `/etc/timezone` is `Etc/UTC` |
| Final IMG network time | Chrony is installed/enabled for Online, retains network sources and `makestep`, and has no active RTC-sync/calibration directive |
...
## RTC/timezone implementation in the final IMG
...
- Secure Boot/runtime boot of this new launcher-built IMG on the physical Acer.
